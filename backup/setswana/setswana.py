...
def categorize_sentences(sentences):
    setswana_clean_text = []
    setswana_text_with_numbers = []

    for sentence in sentences:
        sentence = sentence.strip()
        if sentence:
            if any(char.isdigit() for char in sentence) and any(char.isalpha() for char in sentence):
                setswana_text_with_numbers.append(sentence)
            elif is_coherent(sentence):
                setswana_clean_text.append(sentence)

    categorized_text = []
    categorized_text.append("Clean Text:")
    categorized_text.extend(setswana_clean_text)
    categorized_text.append("\nText with Numbers:")
    categorized_text.extend(setswana_text_with_numbers)
...
def main():
    input_file = "./setswana/setswana.txt"
    setswana_clean_text_file = "./setswana/setswana_clean_text.txt"
    setswana_text_with_numbers_file = "./setswana/setswana_text_with_numbers.txt"
    ...
    create_text_file(setswana_clean_text_file, categorized_text[1:categorized_text.index("\nText with Numbers:")])
    create_text_file(setswana_text_with_numbers_file, categorized_text[categorized_text.index("\nText with Numbers:") + 1:])
