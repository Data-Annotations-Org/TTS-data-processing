...
def categorize_sentences(sentences):
    sepedi_clean_text = []
    sepedi_text_with_numbers = []

    for sentence in sentences:
        sentence = sentence.strip()
        if sentence:
            if any(char.isdigit() for char in sentence) and any(char.isalpha() for char in sentence):
                sepedi_text_with_numbers.append(sentence)
            elif is_coherent(sentence):
                sepedi_clean_text.append(sentence)

    categorized_text = []
    categorized_text.append("Clean Text:")
    categorized_text.extend(sepedi_clean_text)
    categorized_text.append("\nText with Numbers:")
    categorized_text.extend(sepedi_text_with_numbers)
...
def main():
    ...
    create_text_file(sepedi_clean_text_file, categorized_text[1:categorized_text.index("\nText with Numbers:")])
    create_text_file(sepedi_text_with_numbers_file, categorized_text[categorized_text.index("\nText with Numbers:") + 1:])
